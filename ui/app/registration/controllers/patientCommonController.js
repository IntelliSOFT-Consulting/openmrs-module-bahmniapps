...
angular.module("bahmni.registration").controller("PatientCommonController", [
  "$scope",
  "$rootScope",
  "$http",
  ...
  function (
    ...
    $scope.showBirthTime =
      appService.getAppDescriptor().getConfigValue("showBirthTime") != null
        ? appService.getAppDescriptor().getConfigValue("showBirthTime")
        : true; // show birth time by default
    ...
    $scope.showIdentifierVerificationButton = function (identifierType, identifierValue) {
      ...
      if (
        extenstionPoint != null &&
        identifierValue === undefined &&
        _.some($rootScope.currentUser.privileges, { name: extenstionPoint.extensionParams.requiredPrivilege })
      ) {
        if (
          identifierExtnMap.get(extenstionPoint.id) === identifierType ||
          identifierExtnMap.get(extenstionPoint.id) === undefined
        ) {
      ...
      for (var key in addressMap) {
        if (address[key] && address[key] !== null) {
          if (key === "line") {
            ...
            $scope.patient.address[addressMap[key]] = address[key];
          }
        }
      }
    }
            ...
            if (changedDetails.contactPoint != null) {
              for (var i = 0; i < changedDetails.contactPoint.length; i++) {
                ...
                if (contact.system === "phone") {
                  $scope.patient[contactAttribute] = contact.value;
                }
              }
            }
            break;
    ...
    function initPatientNameDisplayOrder() {
      ...
      var valid = _.every(nameFields, function (val) {
    ...
    $scope.getTranslatedPatientControls = function (controls) {
      var translatedName = Bahmni.Common.Util.TranslationUtil.translateAttribute(
        controls,
        Bahmni.Common.Constants.registration,
        $translate
      );
    ...
    $scope.localLanguageNameIsRequired = function (nameType) {
      ...
      if (_.isEmpty(nameType)) {
        return (
          personAttributes.givenNameLocal.required ||
          personAttributes.middleNameLocal.required ||
          personAttributes.familyNameLocal.required
        );
      }
    ...
    $scope.$on("$destroy", function () {
      stateChangeListener();
      if ($scope.duplicateCheckTimeout) {
        clearTimeout($scope.duplicateCheckTimeout);
      }
      // Reset all duplicate-related flags on destroy
      duplicateModalOpen = false;
      userChosenToContinue = false;
    });
    ...
    $scope.getDeathConcepts = function () {
      return $http({
        ...
        transformResponse: [
          function (deathConcept) {
            if (_.isEmpty(deathConcept)) {
              $scope.deathConceptExists = false;
            } else {
              $http
                .get(Bahmni.Common.Constants.conceptSearchByFullNameUrl, {
                  params: {
                    name: deathConcept,
                ...
                .then(function (results) {
                  ...
                  _.forEach(activeDeathConcepts, function (deathConcept, index) {
                    activeDeathConcepts[index] = $scope.updateDisplayFieldToLocaleSpecific(
                      $scope.filterNamesForLocale(
                        deathConcept,
                        $rootScope.currentUser.userProperties.defaultLocale,
                        "FULLY_SPECIFIED"
                      )
                    );
                  });
    ...
    $scope.showCasteSameAsLastName = function () {
      ...
      caste = personAttributeHasCaste
        ? $rootScope.patientConfiguration.attributeTypes[personAttributes.indexOf("caste")].name
        : undefined;
    ...
    $scope.handleUpdate = function (attribute) {
      var ruleFunction =
        Bahmni.Registration.AttributesConditions.rules && Bahmni.Registration.AttributesConditions.rules[attribute];
      if (ruleFunction) {
    ...
    $scope.$watch("patient.familyName", function () {
      ...
      // Also check for duplicates when family name changes
      checkForDuplicates();
    });
    ...
    // Duplicate patient checking functionality
    var duplicateModalOpen = false; // Flag to prevent multiple modals
    var userChosenToContinue = false; // Flag to remember if user chose to continue with duplicate

    var checkForDuplicates = function () {
      // Don't check for duplicates if:
      // 1. Feature is disabled
      // 2. Required fields are missing (all four: given name, family name, gender, date of birth)
      // 3. Modal is already open
      // 4. We're editing an existing patient (when onlyForNewPatients is true)
      // 5. Patient already has a UUID (existing patient)
      // 6. User has already chosen to continue with this patient data
      if (
        ...
        duplicateModalOpen ||
        (DUPLICATE_PATIENT_CONFIG.onlyForNewPatients && !$scope.createPatient) ||
        $scope.patient.uuid ||
        userChosenToContinue
      ) {
        return;
      }
      ...
      $scope.duplicateCheckTimeout = setTimeout(function () {
        // Double-check that modal isn't open and all required fields are present
        if (
          duplicateModalOpen ||
          $scope.patient.uuid ||
          userChosenToContinue ||
        ...
        duplicatePatientService
          .searchForDuplicates($scope.patient)
          .then(function (duplicates) {
            if (duplicates && duplicates.length > 0 && !duplicateModalOpen && !$scope.patient.uuid && !userChosenToContinue) {
              duplicateModalOpen = true;
              showDuplicatePatientModal(duplicates);
            }
          })
          .catch(function (error) {
            console.error("Error checking for duplicates:", error);
          });
      }, DUPLICATE_PATIENT_CONFIG.triggerDelay);
    };

    var showDuplicatePatientModal = function (duplicatePatients) {
      var modalInstance = ngDialog.open({
        template: "views/duplicatePatientModal.html",
        controller: "DuplicatePatientController",
        className: DUPLICATE_PATIENT_CONFIG.modalClassName,
        data: {
          duplicatePatients: duplicatePatients,
          currentPatient: $scope.patient,
        },
        closeByDocument: DUPLICATE_PATIENT_CONFIG.closeByDocument,
        closeByEscape: DUPLICATE_PATIENT_CONFIG.closeByEscape,
      });

      modalInstance.closePromise.then(function (data) {
        ...
        if (data.value === "cancel") {
          ...
          messagingService.showMessage("info", "REGISTRATION_CANCELLED_MESSAGE");
        } else if (data.value === "continue") {
          // User chose to continue with new patient - remember this choice
          userChosenToContinue = true;
          messagingService.showMessage("info", "REGISTRATION_CONTINUING_WITH_NEW_PATIENT");
        }
        // If a patient was selected, navigation already happened in the modal controller
      });
    };

    // Set up watchers for the fields that trigger duplicate checking
    // Combined watcher for better coordination and fewer duplicate calls
    $scope.$watchGroup(
      ["patient.givenName", "patient.familyName", "patient.gender", "patient.birthdate"],
      function (newValues, oldValues) {
        ...
        for (var i = 0; i < newValues.length; i++) {
          if (newValues[i] !== oldValues[i]) {
            ...
            // If core identifying fields changed significantly, reset the continue flag
            if (i === 0 || i === 1 || i === 2 || i === 3) {
              // any of the four key fields
              var oldVal = oldValues[i] || "";
              var newVal = newValues[i] || "";

              // Consider it significant if more than 2 characters different in names or any change in gender/birthdate
              if ((i === 0 || i === 1) && Math.abs(newVal.length - oldVal.length) > 2) {
                // Name length change > 2
                hasSignificantChanges = true;
              } else if (i === 2 || i === 3) {
                // Gender or birthdate change
                hasSignificantChanges = true;
              }
            }
          }
        ...
        // Reset continue flag if significant changes made
        if (hasSignificantChanges && userChosenToContinue) {
          userChosenToContinue = false;
        }

        // Only trigger duplicate check if ALL required fields are present: givenName, familyName, gender, birthdate
        if (hasChanges && newValues[0] && newValues[1] && newValues[2] && newValues[3]) {
          checkForDuplicates();
        }
      }
    );
